...
public class NotesCont : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //ハートに触れた場合のチェック
        if (!notesManager.CanInputKeyHeart() && notesObjects[0].transform.position.x >= -Heart_range && notesObjects[0].transform.position.x <= Heart_range)
        {
        ...
        if (notesManager.CanInputKeyHeart() && notesManager.playerCanMove && Input.GetKeyDown(KeyCode.W))
        {
            KyeDown();
        }
        if (notesManager.CanInputKeyHeart() && notesManager.playerCanMove && Input.GetKeyDown(KeyCode.A))
        {
            KyeDown();
        }
        if (notesManager.CanInputKeyHeart() && notesManager.playerCanMove && Input.GetKeyDown(KeyCode.S))
        {
            KyeDown();
        }
        if (notesManager.CanInputKeyHeart() && notesManager.playerCanMove && Input.GetKeyDown(KeyCode.D))
        {
            KyeDown();
        }
